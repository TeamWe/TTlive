...
#define REGISTER_URL @"http://192.168.1.140//TTlive/think/public/user/Login/QQLogin"
#define APP_ID @"1105729575"
@interface ViewController ()<TencentSessionDelegate,QQShareDelegate,WBHttpRequestDelegate>
{
...
- (void)viewDidLoad {
    ...
    UIButton *qqLogin = ({
        ...
        button.frame = CGRectMake(80, 500, 100, 100);
    ...
    UIButton *wbLogin = ({
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(180, 500, 100, 100);
        [button addTarget:self action:@selector(ssoButtonPressed) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:button];
        button;
    });
    
    [qqLogin setImage:[UIImage imageNamed:@"login_ico_qq"] forState:UIControlStateNormal];
    [wbLogin setImage:[UIImage imageNamed:@"login_ico_weibo"] forState:UIControlStateNormal];
...
#pragma mark demo for network
- (void)loginAction1:(id)sender{
//    NSDictionary *dic = @{@"123":@"123",nil};
//    NSLog(@" response %@",dic);
    NSString *reBody = [NSString stringWithFormat:@"method=qq&info=%@&openid=%@",@{@"123":@"123"},@"werqwersdf"];
    
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[REGISTER_URL stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:@"`#%^{}\"[]|\\<> "].invertedSet]] cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:30];
    request.HTTPMethod = @"POST";
    request.HTTPBody = [reBody dataUsingEncoding:NSUTF8StringEncoding];
    
    NSURLSession *session = [NSURLSession sharedSession];
    NSURLSessionTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        NSLog(@"response:%@,error:%@",[NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableLeaves error:nil],error);
    }];
    [task resume];
}
...
- (void)tencentDidLogin{
    ...
    if (_tencentOAuth.accessToken) {
        ...
        //这个方法就是 用户信息的回调方法。
        NSLog(@"****accessToken 获取成功:%@**** openiD is:%@",_tencentOAuth.accessToken,_tencentOAuth.openId);
...
- (void)getUserInfoResponse:(APIResponse*) response{
    ...
    NSString *reBody = [NSString stringWithFormat:@"method=QQ&openId=%@&nickname=%@&gender=%@&province=%@&city=%@&msg=%@&thumbnail=%@&image=%@",_tencentOAuth.openId,dic[@"nickname"],dic[@"gender"],dic[@"province"],dic[@"city"],dic[@"msg"],dic[@"figureurl_qq_1"],dic[@"figureurl_qq_2"]];
    /*
     *同步帐号信息
     *另外提示后台要做是否为空的判断有错误要返回状态码！！！！
     */
...
#pragma mark should sync?   add userId
